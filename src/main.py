...
from src.logger_toolbox import setup_logging
...
def training_loop(start_epoch, end_epoch, optimizer, train, test, logger, n_classes):

    for epoch in range(start_epoch + 1, start_epoch + end_epoch + 1):
        t = time.time()
        train(epoch, optimizer, n_classes)
        test(epoch, n_classes)
...
if __name__ == '__main__':
    ...
    training_loop(0, args.args.epochs, optimizer, train, test, logger, n_classes=d.number_of_classes)
    ...
    training_loop(args.args.epochs, args.args.finetuneepochs, optimizer, train, test, logger, n_classes=d.number_of_classes)
